    ...
    <ul class="nav navbar-nav ml-auto">
        ...
        <li class="nav-item dropdown mr-30">
            ...
            <div class="dropdown-menu dropdown-menu-right">
                ...
                <a class="dropdown-item" href="" onclick="event.preventDefault();document.getElementById('logout-form').submit();"><i class="text-danger ti-unlock"></i>{{ __('Sidebar_trans.Logoff') }}</a>

                @if(auth("web")->check())
                <form id="logout-form" action="{{ route("logout") }}" method="POST" style="display: none;">
                    <ipnut type="hidden" name="type" value="web"/>
                @elseif(auth("techer")->check())
                <form id="logout-form" action="{{ route("logout") }}" method="POST" style="display: none;">
                    <ipnut type="hidden" name="type" value="techer"/>
                @elseif(auth("student")->check())
                <form id="logout-form" action="{{ route("logout") }}" method="POST" style="display: none;">
                    <ipnut type="hidden" name="type" value="student"/>
                @elseif(auth("parent1")->check())
                <form id="logout-form" action="{{ route("logout") }}" method="POST" style="display: none;">
                    <ipnut type="hidden" name="type" value="parent1"/>
                @endif
                  @csrf
                </form>
            </div>
